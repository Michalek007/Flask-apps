...
from app.blueprints.user.views import register, users
...
class TestUser:
    ...
    def register(self, method,  **kwargs):
        return RequestContext.request('/register/', register, query_string=kwargs, method=method)

    def users(self, user_id: int = None):
        return RequestContext.request('/register/', users, arg=user_id)
