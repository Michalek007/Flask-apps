...
from app.blueprints.details.views import app_details, help
...
class TestDetails:
    ...
    def app_details(self):
        return RequestContext.request('/app_details/', app_details)

    def help(self, method=None):
        return RequestContext.request('/help/', help, arg=method)

    # private fixtures
    @pytest.fixture
    def mock_get_project_details(self, monkeypatch):
        class MockProjectDetails:
            metadata = namedtuple('Metadata', 'VERSION PROJECT_NAME')('2.0.0.0', 'Light REST-API')
    ...
    def mock_get_service_methods(self, monkeypatch):
        ...
        def mock_method2():
            ...
            """
            return None
        monkeypatch.setattr(ServiceMethods, 'get', lambda *args, **kwargs: [mock_method1, mock_method2])

    # tests
    def test_app_details(self, mock_get_project_details):
        return_value = self.app_details()
        expected_value = dict(
            ProjectName='Light REST-API',
            Version='2.0.0.0',
