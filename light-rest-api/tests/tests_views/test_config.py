...
from app.blueprints.config.views import get_pid, kill, restart
...
class TestConfig:
    # requests
    def get_pid(self):
        return RequestContext.request('/get_pid/', get_pid)

    def kill(self):
        return RequestContext.request('/kill/', kill)

    def restart(self):
        return RequestContext.request('/restart/', restart)
