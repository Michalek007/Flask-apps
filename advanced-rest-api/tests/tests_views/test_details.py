...
class TestDetails:
    ...
    def mock_get_project_details(self, monkeypatch):
        class MockProjectDetails:
            metadata = namedtuple('Metadata', 'VERSION PROJECT_NAME')('2.0.0.0', 'Advanced REST-API')
    ...
    def mock_get_service_methods(self, monkeypatch):
        ...
        def mock_method2():
            ...
            """
            return None
        monkeypatch.setattr(DetailsClass, 'get_service_methods', lambda *args, **kwargs: [mock_method1, mock_method2])

    # tests
    def test_app_details(self, mock_get_project_details):
        return_value = self.app_details()
        expected_value = dict(
            ProjectName='Advanced REST-API',
            Version='2.0.0.0',
